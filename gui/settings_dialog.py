...
class SettingsDialog(QDialog):
    ...
    def _tab_personal(self):
        ...
        self.edit_groupname.setPlaceholderText('输入分组名')
        layout.addRow('分组:', self.edit_groupname)
    ...
    def _load_to_ui(self):
        ...
        # 昵称和分组以引擎当前值为准（setting.py 是引擎的启动源）
        nickname = self._cfg.get('nickname', '') or setting.NICKNAME
        groupname = self._cfg.get('groupname', '') or setting.GROUPNAME
        self.edit_nickname.setText(nickname)
        self.edit_groupname.setText(groupname)
    ...
    def _on_save(self):
        ...
        new_groupname = self.edit_groupname.text().strip()

        self._cfg['nickname'] = new_nickname
        self._cfg['groupname'] = new_groupname
        ...
        # ── 同步到引擎内存 ──
        setting.NICKNAME = new_nickname
        setting.GROUPNAME = new_groupname
        Instance.me.nickname = new_nickname
        Instance.me.groupname = new_groupname

        # ── 回写 setting.py，保证重启不丢失 ──
        setting_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..', 'core', 'setting.py'))
        with open(setting_path, 'r', encoding='utf-8') as f:
            content = f.read()
        content = re.sub(
            r"^NICKNAME\s*=\s*'[^']*'",
            f"NICKNAME = '{new_nickname}'", content, flags=re.MULTILINE)
        content = re.sub(
            r"^GROUPNAME\s*=\s*'[^']*'",
            f"GROUPNAME = '{new_groupname}'", content, flags=re.MULTILINE)
