...
class Security:
    def __init__(self):
        ...
        logger.info('RSA 密钥: e=%s n=%s...' % (hex(self.pubKey.e)[:20], hex(self.pubKey.n)[:20]))
        self.dictKey = dict()
    ...
    def decrypt(self,encryptkey, encryptMsg):
        logger.info('rsa.decrypt: key=%dbytes, msg=%dbytes, priKey n=%s...' % (
            len(encryptkey), len(encryptMsg), hex(self.priKey.n)[:20]))
        arKey = rsa.decrypt(encryptkey, self.priKey)
        logger.info('rsa 解密得 blowfish key(%dbytes): %s' % (len(arKey), arKey.hex()))
        iv = b'\0\0\0\0\0\0\0\0'
