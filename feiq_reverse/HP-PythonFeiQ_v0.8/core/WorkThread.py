from core.LogHelp import logger
from core import IPMSG
from core.feiqstruct import *
from core.TaskManager import *
from core.UserManager import FeiQUserManager
from core.SecurityManager import SecurtInstance
...
class MainWorkThread(TaskManager):
    ...
    def __init__(self):
        ...
        self.cancel_cb = None    # callback(filename) -> True=取消传输
                ...
                if cmd == IPMSG.IPMSG_GETFILEDATA or cmd == 0x62:  # 96 or 98
                    ...
                    if finfo:
                        ...
                        if cmd == 0x62 and file_type >= 2:
                            ...
                            HEADER_LEN = 0xC00 + 4
                            total_to_send = file_size - HEADER_LEN if file_size > HEADER_LEN else file_size
                            logger.info('GETDIRFILES: 流式发送条目数据 %s (%d字节, 去头%d, 实际%d)' % (
                                filepath, file_size, HEADER_LEN, total_to_send))
                                ...
                                try:
                                    ...
                                    with open(filepath, 'rb') as f:
                                        ...
                                        while True:
                                            ...
                                            sent += len(chunk)
                                            if self.progress_cb:
                                                try: self.progress_cb(finfo['filename'], sent, total_to_send)
                                                except: pass
                                            if self.cancel_cb:
                                                try:
                                                    if self.cancel_cb(finfo['filename']):
                                                        send_cancelled = True
                                                        break
                                                except: pass
                                    # 确保最终 100%
                                    if self.progress_cb:
                                        try: self.progress_cb(finfo['filename'], total_to_send, total_to_send)
                                        except: pass
                                    if send_cancelled:
                                        logger.info('用户取消发送(文件夹): %s' % finfo['filename'])
                                    else:
                            ...
                            try:
                                ...
                                # 确保最终 100% 进度
                                if self.progress_cb and not send_cancelled:
                                    try: self.progress_cb(finfo['filename'], filesize, filesize)
                                    except: pass
                                if send_cancelled:
                                    try: conn.shutdown(2)
                                    except: pass
                                    ...
                                    logger.info('用户取消发送: %s (已传%d/%d字节)' % (
                                        finfo['filename'], total_sent, filesize))
                                else:
                                    logger.info('文件发送完成: %s -> %s (%d字节)' % (
                                        finfo['filename'], addr[0], total_sent))
                            except Exception as se:
                                logger.error('sendall失败(文件%s, 已发%d/%d字节): %s' % (finfo['filename'], total_sent, filesize, str(se)))
                            finally:
                                # 无论成功/失败/取消，单文件都需要清理 pending
                                if file_type == 1:
    ...
    def _download_one_to(self, save_path, ip, packno, file_id, filename, file_size, file_type=1):
        ...
        try:
            ...
            sock.close()
            if self.progress_cb:
                try: self.progress_cb(filename, file_size, file_size)
                except: pass
