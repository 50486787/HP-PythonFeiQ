...
- [x] 图片发送（选择图片文件，显示缩略图预览）
- [x] 设置页面（保存路径、头像昵称、防火墙、开机自启、多网段）
- [x] 开机自启动（注册表 Run 键，可取消）

### 设置对话框

顶栏右侧齿轮图标 ⚙ 点击打开设置窗口，包含：

- **个人信息**：头像（点击更换）、昵称输入框
- **文件**：默认接收保存路径（浏览选择目录）
- **防火墙**："解除防火墙阻止"按钮，通过 `netsh advfirewall` 添加 UDP+TCP 2425 端口入站规则
- **多网段**：可添加额外广播地址（如 192.168.1.255），列表编辑，支持增删
- **开机启动**：复选框开关，写入/删除注册表 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`
- 设置保存到 `gui/settings.json`，启动时加载，不修改 `setting.py`
